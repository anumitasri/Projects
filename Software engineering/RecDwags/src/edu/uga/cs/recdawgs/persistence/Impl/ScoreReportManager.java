            ...
            throws RDException, SQLException
    {
       ...
       if(!ScoreReport.isPersistent()) // is the Club object persistent?  If not, nothing to actually delete
        ...
        public void storeScoreReport(ScoreReport ScoreReport) 
                throws RDException, SQLException
        {
            String               insertScoreReportSql = "insert into ScoreReport ( id,homepoints,awaypoints,date,mid,uid ) values ( ?, ?, ?, ?, ?, ? )";
            String               updateScoreReportSql = "update ScoreReport set id = ?, homepoints = ?, awaypoints = ?, date = ?, mid = ?, uid = ? where id = ?";
            PreparedStatement    stmt = null;
            int                  inscnt;
            long                 ScoreReportId;
            ...
            try {
                ...
                if( ScoreReport.getId()>=0) // name is unique unique and non null
                    stmt.setLong(1,ScoreReport.getId());
                else 
                    throw new RDException( "ScoreReport.save: can't save a ScoreReport: name undefined" );

                if( ScoreReport.getHomePoint()>=0 )
                    stmt.setInt( 2, ScoreReport.getHomePoint());
                else
                    stmt.setNull( 2, java.sql.Types.INTEGER);
                if( ScoreReport.getAwayPoints()>=0)
                    stmt.setInt( 3, ScoreReport.getAwayPoints());
                else
           ...
           // String       selectScoreReportSql = "select s.id, s.homepoints, s.awaypoints, s.date, m.mid, u.uid, " +
            ...
            if( ScoreReport != null ) {
                if( ScoreReport.getId() >= 0 ) // id is unique, so it is sufficient to get a person
                    query.append( " and id = " + ScoreReport.getId() );
               // else if( club.getName() != null ) // userName is unique, so it is sufficient to get a person
                 //   query.append( " and name = '" + club.getName() + "'" );
                else {
                	//id, homepoints, awaypoints, date , mid , uid
                    if( ScoreReport.getHomePoint()>=0)
                        condition.append( " and homepoints = " + ScoreReport.getHomePoint()); 
                    if( ScoreReport.getAwayPoints()>=0)
                        condition.append( " and awaypoints = " + ScoreReport.getAwayPoints() );   

                    if( ScoreReport.getDate() != null ) {
                        if( condition.length() > 0 )
                            condition.append( " and" );
                        condition.append( " date = '" + ScoreReport.getDate() + "'" );
                    }
