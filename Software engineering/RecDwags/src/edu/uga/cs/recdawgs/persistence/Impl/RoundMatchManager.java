...
/*	public void Match(Connection conn, ObjectLayer object){
    ...
    public Iterator<Match> restoreRoundMatch( Round round ) throws RDException{
       	...
       	String getMatchRoundLinksSql = "Select m.id, m.home_points, m.away_points, m.mdate, m.isCompleted, m.svid, m.rid, m.hometeamid, m.awayteamid from"
				+"match m, round r";
       	...
       	if(round != null){
       		if(round.isPersistent()){
       			query.append( "where m.rid = " + round.getId() );
       		}
       	}
